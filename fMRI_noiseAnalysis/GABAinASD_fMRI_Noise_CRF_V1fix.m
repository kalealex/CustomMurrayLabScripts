% GABAinASD_fMRI_Noise_CRF_V1fix
% MurrayLab_2016
% created by AMK 10/17/16
...
% mat files to load
mat_file_V1left = 'CRF_V1*fix*_V1*left*.mat';
mat_file_V1right= 'CRF_V1*fix*_V1*right*.mat';
...
for iS = 1:length(subj_dirs)
    for iF = 1:length(fmri_dirs)
        ...
        if exist(use_dir,'dir')
            ...
            mat_file = dir(fullfile(use_dir,mat_file_V1left));
            ...
            % for right V1
            clear mat_file AVLowContrastData LowContrastData AVHighContrastData HighContrastData
            mat_file = dir(fullfile(use_dir,mat_file_V1right));
            % check for multiple matching filenames
            if length(mat_file)>1 % muktiple files
                disp(['*** found multiple mat files for ' subj_dirs{iS} ': ' fmri_dirs{iF}]); % message
                for iM = 1:length(mat_file)
                    disp([num2str(iM) ': ' mat_file(iM).name])
                end
...
switch AV
    case 1
        save_file = fullfile(home_dir,'fMRI_Noise_CRF_V1fix.mat');
    case 0
        save_file = fullfile(home_dir,'fMRI_Noise_CRF_V1fix_nonAV.mat');
end
